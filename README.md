...
| Free mode end-to-end over REST + WebSocket, leaderboards, monitor | **working** | `packages/server`, `packages/monitor` | `npm run e2e` — 3 autonomous agents play 3 hands, 3/3 verified |
...
| Solidity suite: `Shuffle`, `Poker`, `Token`, `RakeSplitter`, `Staking`, `Vault` | **implemented and tested locally** | `contracts/` | see `contracts/README.md` |
...
that runs offline with `npm test` or `npm run e2e`.
...
npm test               # 587 unit/invariant/fuzz tests across the workspace
npm run e2e            # acceptance run: autonomous agents play and get verified
npm run test:contracts # Solidity suite (Hardhat)
...
| M3 | `Shuffle.sol` commit-reveal RNG | **done off-chain; Solidity in `contracts/`** |
| M4 | Token launch on pons + `Vault.sol` fee routing | **contracts implemented, not launched** |
| M5 | `Poker.sol` escrow/settlement + wager mode | **server flow done (local settlement); on-chain adapter awaits a deployment** |
| M6 | `Staking.sol` house-edge pool | **contract implemented, not deployed** |
