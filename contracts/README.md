...
2. **The house edge belongs to the people who stake the token.** The rake (the small cut the house
   takes from each pot) flows to a staking pool, not to a private operator's bank account.
...
| **Staker** | Locks the token in `Staking.sol`. | A share of every pot's rake, in proportion to how much they staked. This is the "house edge owned by the crowd". |
| **Operator** | Runs the off-chain game engine, publishes the shuffle commitments, pays the gas, and posts a bond. | Nothing directly — the rake goes to stakers and the vault, not to the operator. |
...
| **Trader** | Buys and sells the token on the DEX after launch. | Trading rewards, funded by DEX trading fees routed into `Vault.sol`. |
...
| `Poker.sol` | Holds the money. Tables, buy-ins, escrow, pots, payouts, and the rake cut. Also the emergency pause. | It does not evaluate hands. It checks that the *amounts* add up, not who has the better hand. |
| `RakeSplitter.sol` | Takes the rake from `Poker.sol` and splits it between the staking pool and the vault. | It cannot be fed fake rake — only the poker contract may push to it. |
...
| `Vault.sol` | Holds DEX trading fees and splits them between operations and trading rewards. | It is not a bank for player money — that is `Poker.sol`'s escrow. |
| `Token.sol` | The ERC-20 token itself, with signature-based approvals (EIP-2612). | It has a fixed supply by default: no hidden inflation switch. |

Everything else in the folder is either an interface (`src/interfaces/`), a test, or the deploy
script.
...
|---|---|---|
| Rake | **2.5%**, capped at **0.05 token** per pot | The most the house can take from one pot, ever. |
...
| Token supply | **1,000,000,000**, 18 decimals | Fixed. `ownerMint` reverts unless the deployment explicitly set a cap. |
...
owner controls is the slashed-bond pool and the vault, and those are separate balances.
...
Every pot's rake is split between the staking pool and the vault (50/50 by default). The staking
half is shared among stakers in proportion to their stake. There is no interest rate, no emissions
and no printing — the yield is literally the rake, and every distribution is a public event.
...
npm test             # hardhat test             (198 tests, in-process chain only)
...
| `test/support/helpers.ts` | shared deployment fixture, snapshot reset, phase helpers (not a spec file) |
...
2. **Rake split schedule (SRS §11 Q4 / FR-9).** A single owner-configurable `stakingBps` (default
   5000, i.e. 50/50) with the vault taking the remainder, so a distribution can never strand dust.
   The SRS does not fix the split anywhere.
...
6. **Rake transfer style.** `Poker.sol` transfers the rake and then calls
   ...
   required from `Poker`, so a settlement cannot fail on an approval misconfiguration. The splitter
   then holds both legs until a permissionless `sweepStaking` / `sweepVault` (or `sweepAll`) pushes
   them out; `sweepStaking` also calls `Staking.notifyRewards` so the pool accrues against tokens it
   already holds (FR-9.6).
7. **Chips leave escrow when they are committed**, not at settlement. `commitHand` debits the seat's
   ...
   `token.balanceOf(Poker)` always equals the sum of the per-seat escrow — the strongest solvency
   statement available when escrow is per-seat rather than pooled. A void simply credits the recorded
   contributions back (FR-5.6).
...
| Operation | Gas |
|---|---|
| `settleHand`, 6 seats | **239 828** (0.40 % of a 60M block) |
| `settleHand` marginal cost per extra seat | ~2 996 |
| `settleHand`, 2 / 4 seats | 227 832 / 233 830 |
| `deposit` | 109 257 |
| `openHand` (6 seats) | 156 336 |
| `Poker.commitHand` (pot contribution) | 85 838 |
| `Shuffle.commitSeed` (phase 1) | 119 815 |
| `Shuffle.commitDeck` (52-leaf Merkle root, phase 2) | 164 367 |
| `Shuffle.revealCard` (1 card + 10-node proof, phase 3) | 82 910 |
| `cashOut` | 59 203 |

`settleHand` is O(seats) storage work (FR-5.2, NFR-3). The FR-6 phases are each a normal-sized
transaction: phase 2 hashes a 64-leaf tree once, and each card reveal verifies a single 10-node path
— so hidden cards cost the operator roughly 0.9M gas for a full 6-max hand's ~20 published cards,
paid per street rather than per card if desired.
...
* `ReentrancyGuard` on every function that moves tokens (`Poker.deposit`/`cashOut`/`settleHand`/
  ...
  `RakeSplitter.receiveRake`/`sweep*`, `Shuffle.postBond`/`audit`/`void`/`sweepSlashed`).
...
| **Rake** | The house's cut of each pot. 2.5%, capped, and only when a flop is dealt. |
...
| **Wager table** | Real-money table. Buy-in goes into on-chain escrow, cards are dealt by the verifiable shuffle, and players may only withdraw what they own. |
